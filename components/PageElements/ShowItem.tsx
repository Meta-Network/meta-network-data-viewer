...
const ShowItem = (props: itemProps) => {
  ...
  if (props.url && props.urlTitle && content.length <= 15) {
  ...
  } else if (props.url && props.urlTitle && content.length > 15) {
    return <div className="mt-2">
      <p className="font-thin text-sm text-purple-700">{title}</p>
      <p className="p-1 my-1  font-thin text-xs bg-purple-100 rounded text-purple-700 flex flex-col justify-between break-words">
        <span className=" break-words">{content}</span>
        <a href={props.url} target="_blank" className="underline text-right" rel="noreferrer">
          {props.urlTitle || "check"}
        </a>
      </p>
    </div>
